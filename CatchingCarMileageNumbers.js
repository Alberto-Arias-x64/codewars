...
// return 2 if is a interesting number
// return 1 if next a interesting number of 2 milles
// return 0 if is a bored number

function upSequence(number = 0) {
  if (number < 100) return false
  const string = number.toString().split('')
  for (let index = 0; index < string.length - 1; index++) {
    if (Number(string[index]) + 1 !== Number(string[index + 1] == '0' ? '10' : string[index + 1])) return false
  }
  return true
}
function downSequence(number = 0) {
  if (number < 100) return false
  const string = number.toString().split('')
  for (let index = 0; index < string.length - 1; index++) {
    if (Number(string[index]) - 1 !== Number(string[index + 1])) return false
  }
  return true
}
function checkPalindrome(number = 0) {
  if (number < 100) return false
  const forward = number.toString()
  const backward = number.toString().split('').reverse().join('')
  return forward === backward
}
function checkRounded(number = 0) {
  const rest = number.toString().split('')
  rest.shift()
  const zeros = rest.filter((e) => e == '0')
  return zeros.length === rest.length
}
function isInteresting(number, awesomePhrases) {
  if (number < 98) return 0
  if (number > 1000000000) return 0
  if (awesomePhrases.includes(number)) return 2
  if (checkPalindrome(number)) return 2
  if (upSequence(number)) return 2
  if (downSequence(number)) return 2
  if (checkRounded(number)) return 2
  ////////////////////////////////////////////////////////////////////////////
  if (awesomePhrases.includes(number + 1)) return 1
  if (checkPalindrome(number + 1)) return 1
  if (upSequence(number + 1)) return 1
  if (downSequence(number + 1)) return 1
  if (checkRounded(number + 1)) return 1
  ////////////////////////////////////////////////////////////////////////////
  if (awesomePhrases.includes(number + 2)) return 1
  if (checkPalindrome(number + 2)) return 1
  if (upSequence(number + 2)) return 1
  if (downSequence(number + 2)) return 1
  if (checkRounded(number + 2)) return 1
  return 0
}

// console.log(isInteresting(3, [1337, 256]), 0)
//console.log(isInteresting(123456789, [1337, 256]), 2)
//console.log(isInteresting(987654321, [1337, 256]), 2)
