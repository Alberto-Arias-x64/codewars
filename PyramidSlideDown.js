...
function longestSlideDown(pyramid) {
  for (let row = pyramid.length - 1; row > 0; row--) {
    for (let col = 0; col < pyramid[row].length - 1; col++) {
      const res1 = pyramid[row - 1][col] + pyramid[row][col]
      const res2 = pyramid[row - 1][col] + pyramid[row][col + 1]
      pyramid[row - 1][col] = Math.max(res1, res2)
    }
  }
  return pyramid[0][0]
}
// console.log(longestSlideDown([[1], [2, 3], [9, 1, 1]]))
console.log(
  longestSlideDown([
    [75],
    [95, 64],
