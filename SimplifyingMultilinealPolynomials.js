...
function simplify(poly) {
    poly = poly.replace(/\+/gm, ' ')
    poly = poly.replace(/-/gm, ' -')
    const splittedPoly = poly.split(' ')
    const organizedPoly = splittedPoly.map(element => {
        return JSON.stringify(Array.from(element).sort((element_1, element_2) => element_1 > element_2 ? 1 : -1)).replace(/[\[\]",]/gm, '')
    })
    const normalizedList = organizedPoly.map(element => element.replace(/[^a-z]/gm, ''))
    let list = []
    normalizedList.forEach((element_1, index_1) => {
        normalizedList.forEach((element_2, index_2) => {
            if (element_1 === element_2 && index_1 !== index_2) {
                list.push([index_1,index_2])
            }
        })
    })
    list.forEach(element => {
        console.log("first", organizedPoly[element[0]],"// second", organizedPoly[element[1]])
    })
    return list
}
