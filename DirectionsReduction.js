...
// and "SOUTH", "SOUTH" and "EAST" are not directly opposite of each other and can't become such. Hence the result path is itself : ["NORTH", "WEST", "SOUTH", "EAST"].
// if you want to translate, please ask before translating.

function dirReduc(arr) {
    arr.forEach((element, index) => {
        if (element.toLowerCase() === 'north' && arr[index + 1] && arr[index + 1].toLowerCase() === 'south') {
            arr[index] = ''
            arr[index + 1] = ''
        }
        else if (element.toLowerCase() === 'south' && arr[index + 1] && arr[index + 1].toLowerCase() === 'north') {
            arr[index] = ''
            arr[index + 1] = ''
        }
        else if (element.toLowerCase() === 'east' && arr[index + 1] && arr[index + 1].toLowerCase() === 'west') {
            arr[index] = ''
            arr[index + 1] = ''
        }
        else if (element.toLowerCase() === 'west' && arr[index + 1] && arr[index + 1].toLowerCase() === 'east') {
            arr[index] = ''
            arr[index + 1] = ''
        }
    })
