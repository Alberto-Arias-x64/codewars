...
function queueTime(customers = [], n = 0) {
    if (customers.length == 0) return 0
    if (customers.length < n) n = customers.length
    let time = 0
    const ATH = new Array(n).fill(0)
    while (true) {
        ATH.forEach((element, index) => {
            if (element === 0 && customers.length > 0) {
                ATH[index] = customers[0]
                customers.shift()
            }
        })
        const brakeCounter = ATH.filter(element => element === 0)
        if (brakeCounter.length === ATH.length ) break
        ATH.forEach((element, index) => element > 0 ? ATH[index]-- : null)
        time++
    }
    return time
}
